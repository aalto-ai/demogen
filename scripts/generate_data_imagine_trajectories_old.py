...
from gscan_metaseq2seq.util.dataset import PaddingDataset, ReshuffleOnIndexZeroDataset, MapDataset
...
class MonotonicRandomPositionEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        )[None].expand(x.shape[0], -1).to(x.device)
        
        return self.embedding(permutation)


class MaskedLanguageModel(pl.LightningModule):
    def __init__(self, num_words, num_positions, pad_word_idx, eos_word_idx, lr=0.0001, wd=1e-2, emb_dim=128, nlayers=8, nhead=4, dropout=0.1, norm_first=False, decay_power=-1, warmup_proportion=0.14):
        super().__init__()
        self.save_hyperparameters()
        self.embedding_instructions = nn.Embedding(num_words, emb_dim)
        self.positional_encoding = MonotonicRandomPositionEmbedding(num_positions, emb_dim)
    ...
    def forward(self, base_instruction, instruction_mask):
        ...
        encoded_instruction = self.embedding_instructions(masked_instruction) + self.positional_encoding(masked_instruction)
    ...
    def training_step(self, x, idx):
        ...
        instruction_mask = (torch.stack([
            torch.randperm(instruction.shape[-1])
            for _ in range(instruction.shape[0])
        ]) < (instruction.shape[-1] * np.random.uniform(0.0, 0.7))).to(self.device)
    ...
    def validation_step(self, x, idx, dl_idx):
        ...
        instruction_mask = (torch.stack([
            torch.randperm(instruction.shape[-1])
            for _ in range(instruction.shape[0])
        ]) < (instruction.shape[-1] * 0.2)).to(self.device)
        logits = self(instruction, instruction_mask)
...
def sample_from_model(model, instruction, sample_n, noise_level=0.2, device='cpu'):
    ...
    with torch.inference_mode():
        ...
        ]) < (expanded_instruction.shape[-1] * noise_level)).to(device)
        logits = model(expanded_instruction, instruction_mask)

        samples = torch.distributions.Categorical(logits=logits).sample()

        resampled = samples * instruction_mask + expanded_instruction * ~instruction_mask

        return instruction.cpu(), resampled.view(instruction.shape[0], sample_n, -1).cpu(), instruction_mask.view(instruction.shape[0], sample_n, -1).cpu()
...
class InducingPointEncoder(nn.Module):
    ...
    def forward(self, x, key_padding_mask):
        return self.transformer(
            torch.cat([
                x, self.inducing_point[None, None].expand(x.shape[0], 1, -1)
            ], dim=1).transpose(0, 1),
            src_key_padding_mask=torch.cat([
                key_padding_mask, torch.zeros_like(key_padding_mask[:, :1])
            ], dim=-1)
        )[-1]


class InstructionCLIPBCE(pl.LightningModule):
    def __init__(self, num_words, pad_word_idx, lr=0.0001, wd=1e-2, emb_dim=128, nlayers=8, nhead=4, dropout=0.1, norm_first=False, decay_power=-1, warmup_proportion=0.14):
        super().__init__()
        self.save_hyperparameters()
        self.embedding_instructions = nn.Embedding(num_words, emb_dim)
        ...
        self.transformer_encoder_instruction = InducingPointEncoder(emb_dim, nhead, nlayers, norm_first=norm_first, dropout=dropout)
        self.transformer_encoder_state = InducingPointEncoder(emb_dim, nhead, nlayers, norm_first=norm_first, dropout=dropout)
        self.pad_word_idx = pad_word_idx

    def configure_optimizers(self):
        return transformer_optimizer_config(self, self.hparams.lr, weight_decay=self.hparams.wd, decay_power=self.hparams.decay_power, warmup_proportion=self.hparams.warmup_proportion)
    ...
    def forward(self, x):
        ...
        encoded_instruction = encoded_instruction + self.positional_encoding(encoded_instruction)
        
        return (
            self.transformer_encoder_instruction(encoded_instruction, instruction_padding) @
    ...
    def training_step(self, x, idx):
        ...
        state_pad[:, torch.randperm(state_pad.shape[1], device=state_pad.device) < (state_pad.shape[1] * 0.2)] = 1
        state_pad = state_pad.to(torch.bool)

        outer_product = self.forward((instruction, state, instruction_pad, state_pad))
        
        matching_instruction = (
            instruction[None, :].expand(instruction.shape[0], -1, -1) == instruction[:, None].expand(-1, instruction.shape[0], -1)
        ).all(dim=-1).float()

        loss = F.binary_cross_entropy_with_logits(
            outer_product,
            matching_instruction,
            pos_weight = torch.tensor(matching_instruction.flatten().shape[0] / (matching_instruction[matching_instruction == 1].shape[0]), device=self.device, dtype=torch.float)
        )
        self.log("loss", loss)
...
def transformer_predict(transformer_learner, state, instruction, decode_len):
    ...
    decoder_in = torch.tensor(
        transformer_learner.sos_action_idx, dtype=torch.long, device=transformer_learner.device
    ...
    with torch.inference_mode():
        for i in range(decode_len):
            logits.append(
                transformer_learner.decode_autoregressive(decoder_in, encodings, key_padding_mask)[
                    :, -1
                ]
            )
        ...
        decoded_eq_mask = (
            (decoded == transformer_learner.eos_action_idx).int().cumsum(dim=-1).bool()[:, :-1]
        )
        decoded = decoded[:, 1:]
...
def generate_instructions_and_rank(
    ...
    for batch in dataloader:
        ...
        result_set_states = np.concatenate([
            np.repeat(s[None], len(result_set), axis=0) for s, result_set in zip(state, result_sets)
        ])

        result_sets_concat = np.concatenate([
            np.stack([
                np.array(x) for x in result_sample_set
        ...
        for i in range(result_sets_concat.shape[0] // batch_size + 1):
            ...
            result_set_states_batch = result_set_states[i * batch_size:(i + 1) * batch_size]
            ...
            result_set_states_batch = torch.from_numpy(result_set_states_batch).to(device)
            ...
            with torch.inference_mode():
                encoded_state = instruction_clip_model.state_encoder(result_set_states_batch)
                projected_state = instruction_clip_model.state_encoder_projection(encoded_state)
                encoded_instruction = instruction_clip_model.embedding_instructions(result_set_batch)
                encoded_instruction = encoded_instruction + instruction_clip_model.positional_encoding(encoded_instruction)

                decoded_instruction = instruction_clip_model.transformer_encoder_instruction(encoded_instruction, instruction_pad)
                decoded_state = instruction_clip_model.transformer_encoder_state(projected_state, state_pad)

                # Take the componentwise product
                scores = (decoded_instruction * decoded_state).sum(dim=-1)

            predicted_targets, logits = transformer_predict(transformer_prediction_model, result_set_states_batch, result_set_batch, decode_len)
            
            # Now we populate per_id_results with the score
            # and the result_set_ids
            for batch_id, sample_result, predicted_target, score in zip(result_set_ids_batch.cpu(), result_set_batch.cpu(), predicted_targets.cpu(), scores.cpu()):
                batch_id = batch_id.item()
                if not (sample_result == instruction[batch_id]).all():
                    per_id_results[batch_id].append((sample_result.numpy(), predicted_target.numpy(), score.item()))
        ...
        # and their scores
        for batch_id, sample_scores in sorted(per_id_results.items(), key=lambda x: x[0]):
            yield (
                instruction[batch_id].numpy(), targets[batch_id].numpy(), state[batch_id].numpy(), state[batch_id].numpy(), [s[0] for s in sample_scores], [s[1] for s in sample_scores], [s[2] for s in sample_scores]
            )


def train_mlm(balanced_training_data, valid_demonstrations_dict, seed, mlm_iterations, pad_word, sos_word, vocab_size, batch_size, device='cpu'):
    dataset = ReshuffleOnIndexZeroDataset(
        PaddingDataset(
            MapDataset(balanced_training_data, lambda x: (x[0], )),
            (8, ),
            (pad_word, )
        )
    )
    ...
    valid_dataloaders = [
        DataLoader(Subset(PaddingDataset(
            MapDataset(data, lambda x: (x[0], )),
            (8, ),
            (pad_word, )
        ), np.random.permutation(len(data))[:512]), batch_size=train_batch_size, pin_memory=True)
    ...
    trainer = pl.Trainer(
        logger=[
            ...
            LoadableCSVLogger(
                logs_root_dir, flush_logs_every_n_steps=10
            ),
        ],
...
def train_clip(balanced_training_data, valid_demonstrations_dict, seed, clip_iterations, pad_word, vocab_size, batch_size, device='cpu'):
    nlayers = 4
    nhead = 8
    hidden_size = 128
    dropout_p = 0.1
    ...
    clip_valid_dataloaders = [
        DataLoader(PaddingDataset(
            MapDataset(data, lambda x: (x[0], x[2])),
            (8, None),
            (pad_word, None)
...
def main():
    ...
    parser.add_argument("--load-mlm", type=str)
    parser.add_argument("--save-mlm", type=str)
    parser.add_argument("--load-transformer", type=str, required=True)
    parser.add_argument("--load-clip", type=str)
    parser.add_argument("--save-clip", type=str)
    ...
    (
        ...
        args.training_data, args.dictionary
    )
    ...
    min_len = min([(cmd, len(x)) for cmd, x in training_data_indices_by_command.items()], key=lambda x: x[-1])[-1]
    balanced_training_data = list(itertools.chain.from_iterable([
        [train_demonstrations[i] for i in x[:min_len]]
        for x in training_data_indices_by_command.values()
    ]))

    model = train_mlm(balanced_training_data, valid_demonstrations_dict, seed, 0 if args.load_mlm else mlm_iterations, pad_word, WORD2IDX['[sos]'], len(WORD2IDX), args.batch_size, device=args.device)
    ...
    instruction_clip = train_clip(balanced_training_data, valid_demonstrations_dict, seed, 0 if args.load_clip else clip_iterations, pad_word, len(WORD2IDX), args.batch_size, device=args.device)
    ...
    transformer_model_weights = torch.load(args.load_transformer)
    ...
    transformer_model_trainer.validate(
        ...
        [
            ...
            ), np.random.permutation(512)), batch_size=args.batch_size, pin_memory=True)
            for data in valid_demonstrations_dict.values()
        ]
    )
    ...
    dataloader_splits = {
        split: DataLoader(
            PaddingDataset(
                Subset(demos, np.arange(
                    min(args.offset, len(demos)),
                    min(args.offset + (len(demos) if not args.limit else args.limit), len(demos))
                )),
                (8, 128, (36, 7)),
                (pad_word, pad_action, 0)
            ),
        ...
        for split, demos in zip(
            ...
            itertools.chain.from_iterable([valid_demonstrations_dict.values(), [train_demonstrations]]),
        )
    ...
    for split, dataloader in tqdm(dataloader_splits.items()):
        ...
        for i, batch in enumerate(batched(generate_instructions_and_rank(
            model,
            instruction_clip,
            transformer_model,
            tqdm(dataloader),
            ...
            with open(os.path.join(args.data_output_directory, split, f"{i}.pb"), "wb") as f:
                pickle.dump(batch, f)
