...
class XFormerEncoder(nn.Module):
    ...
    def forward(self, src, src_key_padding_mask=None):
        return self.encoder(src.transpose(0, 1), mask=~src_key_padding_mask).transpose(0, 1)
...
class XFormerDecoder(nn.Module):
    ...
    def forward(self,
                ...
                tgt_mask=None):
        return self.decoder(tgt.transpose(0, 1),
                            context=memory.transpose(0, 1),
                            mask=~tgt_key_padding_mask,
                            context_mask=~memory_key_padding_mask).transpose(0, 1)
...
class StateEncoderTransformer(nn.Module):
    def __init__(
        ...
        self.transformer_encoder = XFormerEncoder(
