...
class RootConj(Rule):
    def __init__(self, max_recursion=0):
        super().__init__(
            lhs=ROOT, rhs=[VP, Terminal("and"), ROOT], max_recursion=max_recursion
        )

    def instantiate(self, left_child, right_child, **kwargs):
        return LogicalForm(
            variables=left_child.variables + right_child.variables,
            terms=left_child.terms
            + right_child.terms
            + (Term("seq", (left_child.head, right_child.head)),),
        )
...
class VpWrapper(Rule):
    ...
    def instantiate(self, rb, vp, meta, **kwargs):
        ...
        return LogicalForm(
            variables=vp.variables + bound.variables[1:], terms=vp.terms + bound.terms
        )
...
class VpIntransitive(Rule):
    ...
    def instantiate(self, vv, dp, meta, **kwargs):
        ...
        return LogicalForm(
            variables=vv.variables + dp.variables, terms=vv.terms + dp.terms + (role,)
        )
...
class NpWrapper(Rule):
    ...
    def instantiate(self, jj, np, meta=None, **kwargs):
        ...
        return LogicalForm(
            variables=np.variables + bound.variables[1:], terms=np.terms + bound.terms
        )
...
class Template(object):
    ...
    def get_leftmost_nonterminal(self):
        assert (
            self.has_nonterminal()
        ), "Trying to get a NT but none present in this derivation."
...
class Grammar(object):
    ...
    def __init__(self, vocabulary: ClassVar, max_recursion=1, type_grammar="normal"):
        ...
        """
        assert (
            type_grammar in self.RULES
        ), "Specified unsupported type grammar {}".format(type_grammar)
        self.type_grammar = type_grammar
        if type_grammar == "simple_intrans":
            assert (
                len(vocabulary.get_intransitive_verbs()) > 0
            ), "Please specify intransitive verbs."
        elif type_grammar == "simple_trans":
            assert (
                len(vocabulary.get_transitive_verbs()) > 0
            ), "Please specify transitive verbs."
        self.rule_list = self.RULES[type_grammar] + self.lexical_rules(
            vocabulary.get_intransitive_verbs(),
        ...
        self.expandables = set(
            rule.lhs for rule in self.rule_list if not isinstance(rule, LexicalRule)
        )
        self.categories = {
    ...
    def empty_command_statistics():
        return {VV_intransitive: {}, VV_transitive: {}, NN: {}, JJ: {}, RB: {}}
    ...
    def lexical_rules(
        ...
        if self.type_grammar == "adverb" or self.type_grammar == "full":
            rb_rules = [
                LexicalRule(
                    lhs=RB, word=word, sem_type=EVENT, specs=Weights(manner=word)
                )
                for word in adverbs
            ]
            all_rules += rb_rules
        nn_rules = [
            LexicalRule(lhs=NN, word=word, sem_type=ENTITY, specs=Weights(noun=word))
            for word in nouns
        ]
        all_rules += nn_rules
        jj_rules = []
            ...
            jj_rules.extend(
                [
                    LexicalRule(
                        ...
                        word=word,
                        sem_type=ENTITY,
                        specs=Weights(adjective_type=COLOR),
                    )
            ...
            jj_rules.extend(
                [
                    LexicalRule(
                        ...
                        word=word,
                        sem_type=ENTITY,
                        specs=Weights(adjective_type=SIZE),
                    )
    ...
    def sample(self, symbol=ROOT, last_rule=None, recursion=0):
        ...
        if recursion == self.max_recursion - 1:
            nonterminal_rules = [
                rule for rule in nonterminal_rules if rule != last_rule
            ]
        ...
        return Derivation(
            next_rule,
            tuple(
                self.sample(next_symbol, next_rule, next_recursion)
                for next_symbol in next_rule.rhs
            ),
    ...
    def generate_all(
        self, current_template: Template, all_templates: list, rule_use_counter: dict
    ):
        ...
        for rule_leftmost_nonterminal in rules_leftmost_nonterminal:
            ...
            if rule_leftmost_nonterminal in rule_use_counter_copy.keys():
                ...
                if (
                    rule_use_counter[rule_leftmost_nonterminal]
                    >= rule_leftmost_nonterminal.max_recursion
                ):
                    continue
            ...
            next_template = current_template.expand_leftmost_nonterminal(
                rule_leftmost_nonterminal, self.expandables
            )

            # Start a new derivation branch for this RHS.
            self.generate_all(next_template, all_templates, rule_use_counter_copy)

    def form_commands_from_template(
        self, derivation_template: list, derivation_rules: list
    ):
        """
        ...
        for command in all_commands:
            ...
            assert " ".join(derivation.words()) == " ".join(
                command
            ), "Derivation and command not the same."
    ...
    def generate_all_commands(self):
        ...
        for i, (derivation_template, derivation_rules) in enumerate(self.all_templates):
            derivations = self.form_commands_from_template(
                derivation_template, derivation_rules
            )
